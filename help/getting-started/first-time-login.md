...
完成[!UICONTROL Cloud Manager]的一般設定後，您就可以開始使用它了。

>[!NOTE]
>
>根據[!UICONTROL Cloud Manager]中指派的角色和應用程式的狀態，您看到的畫面會與插圖中的畫面不同。
...
1. 如果您的程式尚未設定，請按一下[call-to-action](/help/getting-started/navigation.md#cta)中的&#x200B;**設定程式**，設定您的程式說明並定義關鍵績效指標(KPI)。 如果您的程式已設定，CTA會提供其他後續步驟。

您可以繼續檔案[程式設定](/help/getting-started/program-setup.md)，瞭解設定程式中的後續步驟。

## Cloud Manager 活動 {#activity}

Cloud Manager提供方案活動的整合檢視。 它列出所有 CI/CD 管道執行，包括生產和非生產。 此功能提供有關歷史活動和目前活動的資訊。
