...
* 每個組建都是在新環境中完成的。 組建容器在執行之間不會保留資料。
* Maven使用下列三個命令執行：
...
>[!NOTE]
>
>雖然Cloud Manager未定義`jacoco-maven-plugin`的特定版本，但使用的版本必須至少為`0.7.5.201505241946`。

>[!TIP]
>
>若要瞭解如何使用Cloud Manager API，請參閱下列其他資源：
>
...
Cloud Manager [2023.10.0](/help/release-notes/2023/2023-10-0.md)已開始對組建環境進行滾動更新（完成2023.12.0版本），其中包括對Maven 3.8.8的更新。 Maven 3.8.1中推出的變更是安全性增強，可解決潛在漏洞。 具體而言，Maven 現在預設停用所有不安全的 `http://*` 鏡像，如 [Maven 發行說明](https://maven.apache.org/docs/3.8.1/release-notes.html#cve-2021-26291)中所述。

有些使用者在從使用不安全HTTP連線的Maven存放庫下載成品時，會在建置步驟期間遇到問題。

為了確保更新的版本有流暢的使用體驗，Adobe 建議使用者更新其 Maven 存放庫以使用 HTTPS 而非 HTTP。 此調整可支援產業向安全通訊協定轉變，並維持安全可靠的建置流程。
...
>[!IMPORTANT]
>
>Cloud Manager 2025.06.0不再支援Maven工具鏈。 請注意，包含maven-toolchains-plugin設定的管道失敗，因為`Cannot find matching toolchain definitions.`請使用`.cloudmanager/java-version`檔案來選取JDK 11、17或21。
>
>**移轉指南：**
>
...
您可以選取Oracle 8或Oracle 11作為整個Maven執行的JDK。 此方法會變更用於所有外掛程式的JDK。 因此，支援使用[Apache Maven強制器外掛程式](https://maven.apache.org/enforcer/maven-enforcer-plugin/)來檢查和強制執行Java版本。

若要執行此程式，請在管道使用的Git存放庫分支中建立名為`.cloudmanager/java-version`的檔案。 本檔案可能有的內容為 `11` 或 `8`。 任何其他值會受到忽略。 若指定`11`，則系統使用Oracle 11並將`JAVA_HOME`環境變數設為`/usr/lib/jvm/jdk-11.0.22`。 若指定`8`，則系統使用Oracle 8並將`JAVA_HOME`環境變數設為`/usr/lib/jvm/jdk1.8.0_401`。

## 環境變數 {#environment-variables}

### 標準環境變數 {#standard-environ-variables}

在某些情況下，必須根據有關方案或管道的資訊來改變建置流程。

例如，使用gulp之類的工具進行JavaScript縮制時，請針對開發環境與中繼和生產環境使用不同的縮制等級。
...
在某些情況下，您的建置流程會依據特定的設定變數而定。 這些變數不適合放入Git存放庫中，或者在使用相同分支的管道執行之間需要有所變化。

Cloud Manager 讓這些變數能夠經由 Cloud Manager API 或 Cloud Manager CLI 依據每個管道來設定。 變數會以純文字或加密待用的方式儲存。 在任何一種情況下，都能在建置環境中以環境變數的形式使用變數，然後可以從 `pom.xml` 檔案或其他建置指令碼中進行參照。
...
為了正常運作，部分組建需要安裝其他系統套件。 例如，組建會叫用Python或Ruby指令碼，並需要安裝適當的語言解譯器。 呼叫[`exec-maven-plugin`](https://www.mojohaus.org/exec-maven-plugin/)以叫用APT可以處理此案例。 此執行會包裝在Cloud Manager專屬的Maven設定檔中。 例如，若要安裝 Python，您可以執行以下操作：
...
此方法也可用來安裝特定語言的套件。 亦即，使用`gem`作為RubyGems或`pip`作為Python套件。
