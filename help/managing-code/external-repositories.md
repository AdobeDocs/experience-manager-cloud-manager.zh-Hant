...
1. 在每個對應欄位中，提供關於存放庫的下列詳細資料：
   ...
   | **存放庫 URL** | 必要。 存放庫的 URL。<br><br>如果您使用GitHub託管的存放庫，路徑必須以`.git`結尾。<br>例如，*`https://github.com/org-name/repo-name.git`* （URL路徑僅供圖解使用）。<br><br>如果您使用外部存放庫，它必須使用下列URL路徑格式：<br>`https://git-vendor-name.com/org-name/repo-name.git`<br>或<br>`https://self-hosted-domain/org-name/repo-name.git`<br>並符合您的Git廠商。 |
...
1. 選取&#x200B;**儲存**&#x200B;以新增存放庫。
   ...
   若要驗證外部存放庫的所有權，請提供存取權杖。

1. 若要驗證外部存放庫的所有權以便存取，請在&#x200B;**私人存放庫所有權驗證**&#x200B;對話方塊中提供存取權杖，然後按一下&#x200B;**驗證**。
...
1. 在頁面的左上角，按一下[顯示]功能表圖示](https://spectrum.adobe.com/static/icons/workflow_18/Smock_ShowMenu_18_N.svg)以顯示左側功能表。![
...
1. 若要傳送Cloud Manager所需的事件，請設定webhook。 請使用下表來判斷您的Git提供者適用的正確事件。
...
建立檢查時，它看起來像下面的熒幕擷圖。 與`GitHub.com`的主要差異在於`GitHub.com`使用檢查執行，而GitHub Enterprise （使用個人存取權杖）會產生認可狀態：
...
GitLab互動僅仰賴評論。 驗證開始時，會新增註解。 驗證完成後（無論成功還是失敗），初始註解都將被移除並替換為包含驗證結果或錯誤詳細資料的新註解。
...
![程式碼品質驗證完成時](/help/managing-code/assets/repository-webhook-gitlab2.png)
