...
有100多條規則結合了通用Java規則和AEM特定規則。 部分AEM特定規則是根據AEM Engineering的最佳實務建立的，並被稱為[自訂程式碼品質規則](/help/using/custom-code-quality-rules.md)。

>[!TIP]
>
>您可以使用此連結](/help/assets/CodeQuality-rules-latest-AMS.xlsx)下載規則[的完整清單。
...
>[!NOTE]
>
>若要進一步瞭解[!UICONTROL Cloud Manager]執行的自訂程式碼品質規則，請參閱[自訂程式碼品質規則](custom-code-quality-rules.md)。
...
您可以在[管道組態](/help/using/production-pipelines.md)的&#x200B;**測試**&#x200B;索引標籤上，從一組到全部三組中進行選擇。 流量的分佈會根據所選的組數。 也就是說，如果選取全部三組，則會將總頁面檢視次數的33%放入每組。 如果選取兩組，則 50% 流向每組。如果選取了一組，則 100% 的流量將流向該組。
...
Cloud Manager會使用CSE設定的使用者名稱和密碼在作者執行個體上建立資料夾。 然後使用開放原始程式碼資料庫將資產上傳到檔案夾。Assets測試步驟執行的測試是使用[開放原始碼程式庫](https://github.com/adobe/toughday2)撰寫。 在30分鐘的測試期間內會對每個資產的處理時間以及各種系統層級量度進行測量。 此功能可上傳影像和 PDF 文件。
