...
| 重複的行 | 定義為重複區塊中包含的行數。 在以下條件下，會將程式碼區塊視為重複。<br>非Java專案：<ul><li>應該至少有 100 個連續和重複的權杖。</li><li>這些權杖應至少分佈在： </li><li>COBOL 的 30 行程式碼 </li><li>ABAP 的 20 行程式碼 </li><li>其他語言的 10 行程式碼</li></ul>Java 專案：<ul></li><li> 無論權杖和行的數量如何，應至少有 10 個連續和重複的陳述式。</li></ul>偵測重複時會忽略縮排和字串常值中的差異。 | 資訊 | > 1% |
| 雲端服務相容性 | 識別出的雲端服務相容性問題的數量 | 資訊 | > 0 |

>[!NOTE]
>
>如需更多詳細資訊，請參閱 [SonarQube 的量度定義](https://docs.sonarsource.com/sonarqube-server/user-guide/code-metrics/metrics-definition)。
...
品質掃描流程並非完美無瑕，會錯誤地識別出不構成問題的問題。 這種情況便是誤判。

在這些情況下，可以使用標準 Java `@SuppressWarnings` 註解來標註原始程式碼，將規則 ID 指定為註解屬性。 例如，一種常見的誤判是用於偵測硬式編碼密碼的SonarQube規則可能對如何偵測硬式編碼密碼過於敏感。
...
| `Sling`預設GET servlet受到保護，不會遭受DOS攻擊。 | `Sling Get` servlet | 嚴重 |
...
Cloud Manager會根據&#x200B;**企業所有者**&#x200B;角色所設定的KPI （回應時間和每分鐘的頁面檢視次數），建立虛擬使用者或容器。 這些 KPI 在[建立或編輯程式時](/help/getting-started/program-setup.md)便已設定。

根據定義的KPI，最多會建立十個模擬實際使用者的容器。 為了進行測試所選取的頁面將被分割並分配給每個虛擬使用者。
...
* **其他即時頁面** — 確保對前25個不受歡迎但對測試卻很重要的熱門即時頁面以外的頁面進行測試。 和熱門的即時頁面類似，這些頁面都是從存取紀錄中擷取，而且也必定會出現在中繼環境中。

* **新頁面** — 測試新頁面，這些新頁面僅部署到中繼環境，但尚未部署到生產環境，但必須進行測試。
...
若要瞭解如何使用API，請參閱[修補使用者管道變數](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api#operation/patchPipelineVariables) API檔案。
...
此方法會保留在作者執行個體中，其許可權不變。 更改或刪除可能會導致資產效能測試失敗。

#### 用於測試的影像和資產 {#assets-for-testing}

客戶可上傳他們自己的資產供測試使用。 這項流程可在&#x200B;**管道設定**&#x200B;或&#x200B;**編輯**&#x200B;畫面完成。 支援常見影像格式，例如JPEG、PNG、GIF、BMP和Adobe格式。
...
例如，如果分割率為70/30，且每分鐘有10個資產，則會上傳7個影像和3個檔案。
...
>[!TIP]
>
>若要深入瞭解，請參閱[設定生產管道](/help/using/production-pipelines.md)。 若要瞭解如何設定方案及定義您的KPI，請參閱[方案設定](/help/getting-started/program-setup.md)。
...
最佳化的關鍵在於輸出單一「全」套件的專案。 此套件包含由組建產生的其他內容套件，這些內容套件標示為已略過。 當 Cloud Manager 偵測到這種情況時，會直接掃描個別內容套件並根據相依性進行排序，而不是將「全」套件解除封裝。 例如，考慮以下組建輸出。

* `all/myco-all-1.0.0-SNAPSHOT.zip` (content-package)
* `ui.apps/myco-ui.apps-1.0.0-SNAPSHOT.zip` (skipped-content-package)
* `ui.content/myco-ui.content-1.0.0-SNAPSHOT.zip` (skipped-content-package)

如果`myco-all-1.0.0-SNAPSHOT.zip`內的專案只有兩個略過的內容封裝，則會掃描兩個內嵌封裝，而非「全」內容封裝。
