...
>[!NOTE]
>
>在此提供的程式碼範例僅供說明用途。 若要瞭解其概念和品質規則，請參閱[SonarQube的概唸檔案](https://docs.sonarsource.com/sonarqube-server)。
...
### 請勿使用由外部控制的格式字串 {#do-not-use-format-strings-which-may-be-externally-controlled}
...
使用來自外部來源的格式字串 (例如請求參數或使用者產生的內容) 可能會讓應用程式面臨阻絕服務攻擊。 在某些情況下，格式字串受到外部控制，但僅允許來自受信任的來源。
...
從 AEM 應用程式內部執行 HTTP 要求時，務必要設定適當的逾時，以避免不必要的執行緒消耗。 很遺憾，Java™ 的預設 HTTP 用戶端 (`java.net.HttpUrlConnection`) 和廣泛使用的 Apache HTTP Components 用戶端都沒有預設逾時。 因此必須明確設定逾時。 依據最佳做法的要求，上述逾時不應超過60秒。
...
另一個要避免的常見模式是記錄一則訊息後立即擲回例外狀況。 此問題表示例外狀況訊息在記錄檔中重複。
...
一般而言，應該使用 INFO 紀錄層級來區分重要操作，並且預設情況下，會將 AEM 設定為在 INFO 或以上層級記錄。 GET 和 HEAD 方法應僅能唯讀操作，因此不構成重要操作。 在INFO層級記錄以回應GET或HEAD要求會產生顯著的紀錄噪音，使得在紀錄檔中識別有用資訊變得更加困難。 在處理GET或HEAD要求時，如果出現錯誤，請在WARN或ERROR層級記錄。 如需更深入的疑難排解資訊，請登入DEBUG或TRACE層級。
...
依據最佳做法的要求，紀錄訊息應提供有關應用程式中發生例外狀況的位置的內容相關資訊。 雖然您使用棧疊追蹤來確定內容，但紀錄訊息通常更容易讀取和理解。 因此，在記錄例外狀況時，將例外狀況的訊息當成記錄訊息來使用是不好的做法。 例外狀況訊息會詳細說明所發生的問題。 相反地，紀錄訊息會通知讀者當例外發生時，應用程式正在做什麼。 例外狀況訊息仍會記錄。 透過指定您自己的訊息，更容易理解這些紀錄。
...
### Catch區塊中的記錄是在WARN或ERROR層級 {#logging-in-catch-blocks-should-be-at-the-warn-or-error-level}
...
在了解紀錄訊息時，內容極為重要。 使用`Exception.printStackTrace()`只會導致棧疊追蹤輸出到標準錯誤流，省略所有內容。 此外，在像AEM這類多執行緒應用程式中，平行棧疊追蹤輸出會造成問題。 僅能透過記錄架構記錄例外狀況。
...
應該永遠都透過紀錄架構 SLF4J 完成登入 AEM。 直接輸出至標準輸出或標準錯誤串流會遺失記錄架構提供的結構和相關內容資訊，而且有時會導致效能問題。
...
以`/libs`和`/apps`開頭的路徑不是硬式編碼。 這些路徑通常會相對於`Sling`搜尋路徑（預設為`/libs,/apps`）儲存。 使用絕對路徑會引進難以察覺的缺陷，這些缺陷只會在專案生命週期的後期出現。
...
請勿將Sling排程器用於需要保證執行的任務。 Sling已排程的作業可保證執行，且更適合叢集和非叢集環境。

若要瞭解如何在叢集環境中處理Sling作業的詳細資訊，請參閱[Apache Sling事件和作業處理檔案](https://sling.apache.org/documentation/bundles/apache-sling-eventing-and-job-handling.html)。

### 請勿使用AEM已過時的API {#sonarqube-aem-deprecated}
...
通常，會使用標準Java來取代這些API™ *@Deprecated*&#x200B;註解將其識別為`squid:CallToDeprecatedMethod`。

但在某些情況下，API會在AEM的內容中遭到取代，但在其他內容中卻不會遭到取代。 此規則會識別此第二分類。
...
>[!NOTE]
>
>OakPAL 是一種架構，會使用獨立的 Oak 存放庫來驗證內容套件。 AEM合作夥伴、2019年「AEM搖滾明星北美洲」獎得主開發出這款產品。

### 請勿實作或擴充使用@ProviderType註解的產品API {#product-apis-annotated-with-providertype-should-not-be-implemented-or-extended-by-customers}
...
### 請勿在客戶封裝中的`/libs`下建立或編輯節點 {#oakpal-customer-package}
...
客戶應將 AEM 內容存放庫中的 `/libs` 內容樹視為唯讀，這是一個存在已久的最佳做法。 修改 `/libs` 下的節點和屬性都會對大幅和小幅更新產生顯著風險。 Adobe僅透過正式通道對`/libs`進行編輯。
...
類似[套件不應包含重複的OSGi設定規則](#oakpal-package-osgi)，此問題是在多個單獨的內容套件寫入相同節點路徑的複雜專案中常見的問題。 雖然使用內容套件相依性可以確保結果一致，但最好還是完全避免重疊。

### 預設的撰寫模式不是傳統UI {#oakpal-default-authoring}
...
### 具有對話方塊的元件需要觸控式UI對話方塊 {#oakpal-components-dialogs}
...
### 已啟用Proxy的使用者端資料庫中所包含的資源位於名為資源的資料夾中 {#oakpal-resources-proxy}
...
AEM使用者端資料庫可包含影像和字型之類的靜態資源。 如[使用用戶端資料庫文件](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65/content/implementing/developing/introduction/clientlibs#using-preprocessors)所述，使用經 Proxy 處理的用戶端資料庫時，這些靜態資源必須包含在名為 `resources` 的子檔案夾中，以便在發布實例上有效地參考。
...
### 禁止自訂某些標準索引定義 {#oakpal-customizing-ootb-index}
...
AEM Cloud Service禁止未經授權修改以下標準索引：
...
* [Dispatcher設定遺失強制屬性](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---parsing-violation---dispatcher-configuration-missing-mandatory-property)

* [Dispatcher設定過時的屬性](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---parsing-violation---dispatcher-configuration-deprecated-property)

* [找不到Dispatcher設定](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---parsing-violation---dispatcher-configuration-not-found)

* [Httpd設定包含找不到檔案](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---parsing-violation---httpd-configuration-include-file-not-found)
...
* [Dispatcher發佈伺服器陣列快取已啟用`serveStaleOnError`](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-should-have-servestaleonerror-enabled)

* [Dispatcher發佈伺服器陣列篩選器應包含來自AEM原型6.x.x版本的預設拒絕規則](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-filters-should-contain-the-default-deny-rules-from-the-6xx-version-of-the-aem-archetype)

* [Dispatcher發佈伺服器陣列快取`statfileslevel`屬性為>= 2](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-statfileslevel-property-should-be--2)

* [Dispatcher發佈伺服器陣列`gracePeriod`屬性為>= 2](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-graceperiod-property-should-be--2)

* [每個Dispatcher陣列都有唯一的名稱](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---each-dispatcher-farm-should-have-a-unique-name)

* [Dispatcher發佈伺服器陣列快取的`ignoreUrlParams`規則已設定為允許清單](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-should-have-its-ignoreurlparams-rules-configured-in-an-allow-list-manner)

* [Dispatcher發佈伺服器陣列篩選器會以允許清單的方式指定允許的Sling選擇器](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-filters-should-specify-the-allowed-sling-selectors-in-an-allow-list-manner)

* [Dispatcher發佈伺服器陣列篩選器會以允許清單的方式指定允許的Sling尾碼模式](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-filters-should-specify-the-allowed-sling-suffix-patterns-in-an-allow-list-manner)

* [請勿在具有根目錄路徑的「VirtualHost目錄」區段中使用「要求所有被授予」指令](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-require-all-granted-directive-should-not-be-used-in-a-virtualhost-directory-section-with-a-root-directory-path)
