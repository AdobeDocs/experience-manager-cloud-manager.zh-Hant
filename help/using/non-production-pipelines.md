...
>[!NOTE]
>
>您必須等到與管道相關的Git存放庫具有至少一個分支且[程式設定](/help/getting-started/program-setup.md)完成時，才能設定管道。 若要瞭解如何在Cloud Manager中新增和管理存放庫，請參閱[Cloud Manager存放庫](/help/managing-code/managing-repositories.md)。
...
|  | **正在測試** | 僅在編輯非生產管道時可見。<br>UI會顯示管道在程式碼品質驗證過程中執行的測試類別。<ul><li>**靜態程式碼測試** — 分析程式碼的品質和正確性問題。<li>**負載/效能測試** — 評估管線測試中與效能相關的行為。<li>**安全性測試** — 檢查程式碼和管道輸出中的安全性相關問題。 |
...
|  | **中繼部署後核准**&#x200B;核取方塊 | 僅在編輯非生產管道時可見。<br>選取此選項以在部署到中繼環境後需要核准，管道才能繼續。 如果未選取此選項，則管道會根據設定的行為繼續。 |
...
|   | **正在測試** | 僅在編輯非生產管道時可見。<br>UI會顯示管道在程式碼品質驗證過程中執行的測試類別。<ul><li>**靜態程式碼測試** — 分析程式碼的品質和正確性問題。<li>**負載/效能測試** — 評估管線測試中與效能相關的行為。<li>**安全性測試** — 檢查程式碼和管道輸出中的安全性相關問題。</li></ul> |
| **部署選項** | **部署觸發程式** | <ul><li>**手動** - 讓您能手動啟動管道。<li>**在 Git 變更時** - 當認可版本新增到所設定的 Git 分支時啟動管道。 使用此選項，您仍然可以在必要時手動啟動管道。 |
...
>[!NOTE]
>
>如果所選環境已存在網頁層設定管道，則此選項會停用。 在任何時候，每個環境只能有一個 Web 層配置管道。
...
請參閱[新增非生產管道](#add-non-production-pipeline)以啟用智慧建置。
