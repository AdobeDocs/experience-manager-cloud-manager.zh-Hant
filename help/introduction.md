...
若要瞭解適用於AMS的Cloud Manager (Adobe Managed Services)以及它如何讓組織在雲端中自行管理Adobe Experience Manager，請從這裡開始。
...
Adobe Experience Manager 的 [!UICONTROL Cloud Manager] 使開發人員能夠透過以 Adob&#x200B;&#x200B;e Experience Manager 最佳做法為基礎所建置的精簡工作流程建立有影響力的客戶體驗。 針對Adobe Experience Manager最佳化的CI/CD管道可讓您透過簽入程式碼來合併開發工作流程，然後移至生產就緒。 在建置階段，您的自訂程式碼更新會根據最佳做法進行完整測試，以便為您的客戶提供可靠的應用程式。 Cloud Manager 會使用開放式 API 方法，讓您能夠在不中斷現有流程和工具的情況下與您的系統整合。
...
* 持續整合/持續傳遞(CI/CD)程式碼，將開發週期從數月/數週縮短至數天/數小時。
* 程式碼檢查、效能測試和安全性驗證會根據最佳實務進行，然後再推送至生產環境，將生產中斷的情況降至最低。
* API 連線能力補足現有的 DevOps 流程。
* 自動縮放功能，可偵測增加容量的需求，並自動布建額外的Dispatcher/發佈區段。
...
[!UICONTROL Cloud Manager]的使用者介面(UI)可讓您輕鬆存取和管理雲端環境，以及Adobe Experience Manager應用程式的CI/CD管道。
...
[!UICONTROL Cloud Manager]的一項重要功能是能夠使用最佳化的CI/CD管道來加速自訂程式碼或更新的傳送，例如在網站上新增元件。

透過[!UICONTROL Cloud Manager] UI，您可以設定並啟動您的CI/CD管道。 此管道的部分作業會執行完整的程式碼掃描，以確保只有高品質的應用程式才能傳遞至生產環境。
...
與部署觸發無關，每次觸發部署時都一定會執行品質檢查，這是 CI/CD 管道執行的一部分。 品質檢查包括程式碼檢查、安全性測試和效能測試，這些功能都以標準功能提供，您或您的合作夥伴無需執行任何操作。

若要了解更多關於部署程式碼和品質檢查，請參閱「[部署程式碼](/help/using/code-deployment.md)」。

## Cloud Manager 的選用功能 {#optional-features-in-cloud-manager}

Cloud Manager提供其他進階功能，可根據您的特定環境設定和需求讓您的專案受益。 若要進一步討論，請洽詢您的客戶成功工程師(CSE)或Adobe代表，以瞭解您是否對這些功能感興趣。

### 自動縮放 {#autoscaling}

當生產環境承受異常高的負載時，[!UICONTROL Cloud Manager]會偵測對額外容量的需求並使用其自動縮放功能自動布建額外容量。

在這種情況下，[!UICONTROL Cloud Manager]會自動觸發自動縮放布建程式，傳送自動縮放事件的通知，並在數分鐘內布建額外容量。 額外容量會布建在生產環境和相同區域，符合執行Dispatcher/發佈節點的系統規格。
...
* 設定過程中會建立一個綠色負載平衡器。 此負載平衡器絕不會變更，而且是您綠色或「測試」URL的目標。
