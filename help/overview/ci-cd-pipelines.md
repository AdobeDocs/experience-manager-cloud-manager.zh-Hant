...
* **[完整棧疊管道](#full-stack-pipeline)** — 部署完整的AEM應用程式程式碼以及HTTPD/Dispatcher設定。
* **[Web層設定管道](#web-tier-config-pipelines)** — 僅部署HTTPD/Dispatcher設定。
...
以下說明完整棧疊管道如何與[Web層設定管道](#web-tier-config-pipelines)互動。

* 如果存在對應的Web層配置管道，則環境的完整棧疊管道會忽略Dispatcher配置。
* 如果環境的對應Web層配置管道不存在，使用者在配置完整棧疊管道時可以包含或忽略Dispatcher配置。

完整堆疊管道可以是程式碼品質管道或部署。

#### 設定完整棧疊管道 {#configure-full-stack}

請參閱[新增生產管道](/help/using/production-pipelines.md#full-stack-code)。
請參閱[新增非生產管道](/help/using/non-production-pipelines.md#add-non-production-pipeline)。

### Web層設定管道 {#web-tier-config-pipelines}

Web層配置管道允許將HTTPD/Dispatcher配置獨佔部署到AEM執行階段，將其與其他程式碼變更分離。 這是一種簡化的管道，為只想部署Dispatcher設定的使用者提供快速變更的有效方法。
...
以下說明Web層設定管道如何與[完整棧疊管道](#full-stack-pipeline)互動。
...
請參閱[新增生產管道](/help/using/production-pipelines.md#web-tier-config)。
請參閱[新增非生產管道](/help/using/non-production-pipelines.md#add-non-production-pipeline)。
...
智慧型組建可用於計畫碼品質和完整棧疊部署管道（開發、暫存、生產）。
...
* **重要** - 閘道識別出的重要問題，並會導致管道進入暫停狀態。 部署管理員、專案管理員或業務負責人可以覆寫此問題，讓管道繼續。 或者，他們可以接受這些問題，導致管道因失敗而停止。
